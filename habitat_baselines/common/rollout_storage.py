...
class RolloutStorageOracle:
    ...
    def __init__(
        ...
        # For implicit agent only
        self.saved_impl_queries_res = [[] for _ in range(num_envs)]
        self.saved_map_latent_codes = [[] for _ in range(num_envs)]
        self.num_envs = num_envs

        self.observations["goals_rel_pos"] = torch.zeros(num_steps + 1, num_envs, 2)

        self.observations["perfect_gt_implicit_obj"] = torch.zeros(
            num_steps + 1, num_envs, 3
        )
        self.observations["perfect_gt_implicit_pos"] = torch.zeros(
            num_steps + 1, num_envs, 3, 1, 3
        )
    ...
    def insert(
        ...
        # For implicit agent only
        if saved_impl_queries_res is not None:
            for i in range(len(saved_impl_queries_res)):
                self.saved_impl_queries_res[i].append(saved_impl_queries_res[i])
        if saved_map_latent_codes is not None:
            for i in range(len(saved_map_latent_codes)):
                self.saved_map_latent_codes[i].append(saved_map_latent_codes[i])
    ...
    def after_update(self):
        ...
        # For implicit agent only
        self.saved_impl_queries_res = [[] for _ in range(self.num_envs)]
        self.saved_map_latent_codes = [[] for _ in range(self.num_envs)]
    ...
    def recurrent_generator(self, advantages, num_mini_batch):
        ...
        for start_ind in range(0, num_processes, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ind = perm[start_ind + offset]
                if len(self.saved_impl_queries_res[0]) > 0:
                    saved_impl_queries_res_batch.append(
                        self.saved_impl_queries_res[ind][: self.step]
                    )

                if len(self.saved_map_latent_codes[0]) > 0:
                    saved_map_latent_codes_batch.append(
                        self.saved_map_latent_codes[ind][: self.step]
                    )
