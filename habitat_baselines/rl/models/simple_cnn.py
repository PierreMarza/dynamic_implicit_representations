...
class SegHead(nn.Module):
    def __init__(self, additional_convs):
        super().__init__()
        self.additional_convs = additional_convs
        if additional_convs:
            self.conv1 = nn.Sequential(nn.Conv2d(32, 32, 5, padding=2), nn.ReLU(True))
            self.conv2 = nn.Sequential(nn.Conv2d(32, 32, 5, padding=2), nn.ReLU(True))

        self.conv3 = nn.Conv2d(32, 9, 3, padding=1)

    def forward(self, x):
        if self.additional_convs:
            x = self.conv1(x)
            x = self.conv2(x)
        out = self.conv3(x)
        return x, out
